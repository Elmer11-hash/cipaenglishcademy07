...
function filterArticles(category) {
  const items = document.querySelectorAll(".notice-item2nd, .notice-article");

  items.forEach((item) => {
    if (category === "all" || item.dataset.category === category) {
      item.style.visibility = "visible"; // Make items visible
      item.style.opacity = "1"; // Fully visible
      item.style.height = "auto"; // Reset height for visible items
      item.style.marginTop = "0"; // Reset margin
    } else {
      item.style.visibility = "hidden"; // Hide items visually
      item.style.opacity = "0"; // Make items invisible
      item.style.height = "0"; // Collapse height
      item.style.marginTop = "0"; // Keep margin consistent
    }
  });
}
